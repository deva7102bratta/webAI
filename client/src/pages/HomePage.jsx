...
const HomePage = () => {
  ...
  const {user, projects, loadingProjects, generatingProject, loadProjects, handleProjects, handleGenerate, handleDelete, logout} = useAppContext()
  ...
  return (
    <div
      ...
      className="h-screen overflow-y-scroll text-white font-sans bg-cover bg-center bg-no-repeat">
      {/* Nav */}
      <nav className="sticky top-0 z-10 flex items-center justify-between px-6 py-4">
        <div className="flex items-center gap-2">
          <img src={logo} alt="logo" className="size-6"/>
          <span className="text-xl font-semibold tracking-tight">WebAI</span>
        </div>
        <div className="flex items-center gap-4 text-sm font-medium text-zinc-300">
          <span>{user?.name}</span>
          <button onClick={logout} className="py-1.5 px-3 border border-white/20 text-white hover:bg-white/10 text-xs rounded-md cursor-pointer bg-transparent">Sign Out
          </button>
        </div>
      </nav>
                  ...
                  <div key={p._id} className="bg-white/5 border border-white/10 rounded-lg px-4 py-3 flex items-center justify-between group hover:border-white/20 hover:bg-white/10 cursor-pointer backdrop-blur-md transition-all" onClick={()=>navigate(`/builder/${p._id}`)}>
                    ...
                    <div className="flex items-center gap-2">
                      <button 
                        ...
                        className="p-1.5 rounded-md text-zinc-200 hover:text-red-400 hover:bg-white/10">
                        <Trash2Icon size={14}/>
                      </button>
